...
//         if (type !== "IN") {
//             return resp.status(400).json({ status: false, message: "Invalid attendance type. Must be 'IN'." });
//         }

//         // Check if attendance already exists for today
//         connection.query(
//             "SELECT * FROM tbl_attendence WHERE student_id = ? AND destination = ? AND DATE(attendence_date_time) = CURDATE() AND type = ?",
//             [student_id, destination, type],
//             (error, results) => {
//                 if (error) {
//                     return resp.status(500).json({ status: false, message: "Database query failed" });
//                 }
...
//         // Validate input fields for Manual api check
//         if (!student_id || !bus_id || !destination || !type) {
//             return resp.status(400).json({ status: false, message: "All fields are required" });
//         }
...
//         if (destination !== "Home To School") {
//             return resp.status(400).json({ status: false, message: "Invalid destination. Must be 'Home To School'." });
//         }
...
//                     return resp.status(400).json({ status: false, message: "Attendance for 'Home To School - IN' is already recorded today." });
//                 } else {
//                     // Step 2: Insert the new attendance record
//                     connection.query(
//                         "INSERT INTO tbl_attendence (student_id, type, bus_id, destination) VALUES (?, ?, ?, ?)",
//                         [student_id, type, bus_id, destination],
//                         (insertError) => {
...
//         // Step 1: Check if an "IN" record already exists for this student today
//         connection.query(
//             "SELECT * FROM tbl_attendence WHERE student_id = ? AND destination = ? AND DATE(attendence_date_time) = CURDATE() AND type = 'IN'",
//             [student_id, destination],
//             (error, inResults) => {
//                 if (error) {
...
//                 connection.query(
//                     "SELECT COUNT(*) AS count FROM tbl_attendence WHERE student_id = ? AND DATE(attendence_date_time) = CURDATE()",
//                     [student_id],
...
//         // Validate attendance type
//         if (type !== "OUT") {
//             return resp.status(400).json({ status: false, message: "Invalid attendance type. Must be 'OUT'." });
//         }
...
//         connection.query(
//             "SELECT * FROM tbl_attendence WHERE student_id = ? AND destination = ? AND DATE(attendence_date_time) = CURDATE() AND type = 'OUT'",
//             [student_id, destination],
//             (error, outResults) => {
//                 if (error) {
...
//                 connection.query(
//                     "SELECT * FROM tbl_attendence WHERE student_id = ? AND destination = ? AND DATE(attendence_date_time) = CURDATE() AND type = 'IN' ORDER BY attendence_date_time ASC",
//                     [student_id, destination],
//                     (inError, inResults) => {
//                         if (inError) {
...
//                         if (inResults.length === 0) {
//                             return resp.status(400).json({ status: false, message: "Cannot Mark OUT!, Student must be Mark IN for today." });
//                         }
...
//         if (!date || !student_id) {
//             return resp.status(400).json({ error: "Date and student ID are required" });
//         }
//         connection.query(
//             "SELECT destination, attendence_date_time, type FROM tbl_attendence WHERE DATE(attendence_date_time) = ? AND student_id = ?",[date, student_id], (err, results) => {
//                 if (err) {
