...
//             // Step 3: Hash the new password using bcrypt with cost 10
//             bcrypt.hash(admin_password, 10, (err, hashedPassword) => {
//                 if (err) {
//                     return resp.status(500).json({ status: false, message: err.message });
//                 }

//                 // Step 4: Insert the new admin user into the database with the hashed password
//                 const insertQuery = `
//                     INSERT INTO tbl_admin (admin_name, admin_email, admin_password, admin_mobile) 
//                     VALUES (?, ?, ?, ?)
//                 `;

//                 connection.query(insertQuery, [admin_name, admin_email, hashedPassword, admin_mobile], (err, result) => {
//                     if (err) {
//                         return resp.status(500).json({ status: false, message: err.message });
//                     }
...
//     try {
//         // Step 2: Hash the new password using bcrypt with cost 10
//         const hashedPassword = await bcrypt.hash(admin_password, 10);

//         // Step 3: Update the admin_password in the database for the specified admin_email
//         const query = `
//             UPDATE tbl_admin 
//             SET admin_password = ? 
//             WHERE admin_email = ?
...
//         connection.query(query, [hashedPassword, admin_email], (err, result) => {
//             if (err) {
//                 return resp.status(500).json({ status: false, message: err.message });
//             }
...
//     try {
//         connection.query('SELECT * FROM tbl_admin WHERE admin_email = ?', [input.admin_email], async (error, results) => {
//             if (error) {
//                 return resp.status(500).json({ status: false, message: error.message });
//             }
//             if (results.length === 0) {
...
//             const isPasswordValid = await bcrypt.compare(input.admin_password, admin.admin_password);
//             if (!isPasswordValid) {
//                 return resp.status(401).json({
//                     status: false,
//                     message: "Password not matched"
